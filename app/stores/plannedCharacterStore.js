...
export const usePlannedCharacterStore = defineStore('plannedCharacters', () => {
	...
	function setDone(characterName) {
		...
		plannedCharacters.value = objectHelper.omit(
			plannedCharacters.value,
			characterName
		);
		storeToStorage();
			...
			if (activeSkill === 'char') {
				if (character[activeSkill + '_target_level'] !== undefined) {
					let target = gameCharacter
						.charLevellingMaterialsCount(characterName)
						.map((value, key) => {
							return value.level;
						})
						.indexOf(character[activeSkill + '_target_level']);
					let current = gameCharacter
						.charLevellingMaterialsCount(characterName)
						.map((value, key) => {
							return value.level;
						})
						.indexOf(character[activeSkill + '_current_level']);
					if (target > current) {
						return false;
					}
				}
